...
# AS Notes（PKMS、静态网站发布与博客 VS Code 扩展）
...
**AS Notes 将 Markdown 和 `[[wikilink]]` 编辑功能直接带入 [VS Code](https://code.visualstudio.com/) 及兼容编辑器（如 [Antigravity](https://antigravity.google/)、[Cursor](https://cursor.com/)、[Windsurf](https://windsurf.com/)），支持笔记、文档、博客和 Wiki。AS Notes 是 Obsidian 或 Logseq 等 PKMS 应用的编辑器内替代方案。**
...
侧边栏中的**日历**面板显示当前月份及日记指示器。点击任何一天即可打开其日记条目。详情请参阅 [日历](#calendar)。
...
这些命令切换或循环文件 YAML 前置信息中的发布相关字段。详情请参阅 [发布静态网站](#从-as-notes-发布为-htmlhtml-转换)。
...
这些命令仅在当前文件没有 YAML 前置信息块时出现。它们插入一个完整的前置信息模板，带有便于编辑的 tabstop 占位符。

| 命令 | 操作 |
|---|---|
| **Front Matter: Blog Post** | 为博客文章插入前置信息：`title`、`description`、`date`（今天）、`author`、`public: true`、`layout: blog` |
| **Front Matter: Docs Page** | 为文档页面插入前置信息：`title`、`description`、`public: true`、`layout: docs`、`order` |
| **Front Matter: Wiki Page** | 为 Wiki 页面插入前置信息：`title`、`description`、`public: true` |
...
有关内联编辑器设置的完整列表，请参阅 [设置](#settings)。
...
| 自定义日期格式 | `YYYY`、`YY`、`MM`、`DD`、`HH`、`mm`、`ss` 标记的任意组合 | `{{DD/MM/YYYY}}` → `18/03/2026`，`{{DD/MM/YY}}` → `18/03/26` |
...
Pro 用户可以从 **KeePass Password Safe** 侧边栏打开、编辑和创建标准 **KeePass KDBX 4**（`.kdbx`）密码保险库 - 相同的文件也可在 KeePassXC 和其他 KeePass 应用中打开。使用主密码（外加可选的密钥文件）解锁，浏览分组和条目，通过自动清除的剪贴板复制凭据和一次性验证码，并在按需保存的编辑器中编辑条目（字段、自定义字段、标签、验证器密钥、附件、图标、过期时间和历史记录）。新保险库使用 Argon2id 和每个文件随机的盐值。详情及安全注意事项请参阅 [KeePass 密码保险库](https://docs.asnotes.io/keepass-password-safe.html)。
...
根目录的 `package.json` 提供了跨所有包工作的便捷脚本。从仓库根目录：
...
> **注意：** `vs-code-extension` 构建直接打包 `../publish/src/convert.ts`，因此在构建扩展之前，`publish/`（及其对 `common/` 的依赖）必须先安装好依赖。从仓库根目录使用 `npm run install:all` 以正确的顺序安装所有内容。
...
> **前提条件：** `common/` 和 `publish/` 必须先安装好依赖（`npm --prefix common install && npm --prefix publish install`），或从仓库根目录使用 `npm run install:all`。
...
当站点级布局为 `blog`（或在配置中设置了 `blogIndex: true`）时，自动生成的索引页面使用博客感知布局：

1. **最近文章** — 最近的文章（可通过 `recentCount` 配置，默认 3）显示为卡片，带有标题、日期、作者、描述和可选的主图
2. **当前年份** — 当前年份剩余的文章作为紧凑的日期 + 标题列表
3. **年度归档** — 链接到自动生成的往年 `archive-YYYY.html` 页面
4. **无日期** — 前置信息中没有 `date` 的文章列在底部

#### 发布用前置信息字段

| 字段 | 类型 | 描述 |
|---|---|---|
| `public` | boolean | 将页面包含在发布输出中 |
| `title` | string | 页面标题（默认为文件名） |
| `description` | string | 用于 SEO 的元描述 |
| `date` | string | 发布日期（`YYYY-MM-DD`）— 用于博客排序、RSS 和站点地图 |
| `author` | string | 文章作者 — 显示在博客布局和 RSS 源中 |
| `image` | string | 主图/缩略图路径 — 显示在博客布局和博客索引卡片中 |
| `layout` | string | 每页布局覆盖（`docs`、`blog`、`minimal`） |
| `order` | number | 导航排序顺序 |
| `draft` | boolean | 从输出中排除，除非设置了 `--include-drafts` |
| `assets` | boolean | 将引用的图片复制到输出 |
| `retina` | boolean | 启用视网膜图片尺寸 |

#### 博客配置选项

| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `siteTitle` | string | `"AS Notes Site"` | 用于 RSS 源和博客索引的站点标题 |
| `recentCount` | number | `3` | 博客索引上作为卡片显示的最近文章数量 |
| `blogIndex` | boolean | `false` | 使用博客风格的索引生成（当 `layout: "blog"` 时自动启用） |
