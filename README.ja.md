...
# AS Notes（PKMS・静的サイト公開・ブログ用 VS Code 拡張機能）
...
**AS Notes は、ノート、ドキュメント、ブログ、Wiki のための Markdown と `[[wikilink]]` 編集機能を [VS Code](https://code.visualstudio.com/) および互換エディタ（[Antigravity](https://antigravity.google/)、[Cursor](https://cursor.com/)、[Windsurf](https://windsurf.com/) など）に直接統合します。AS Notes は、Obsidian や Logseq のような PKMS アプリのエディタ内代替となります。**

**アイデアを記録し、概念をリンクし、執筆に集中 - エディタを離れることなく。**

AS Notes は、お気に入りの IDE をパーソナルナレッジマネジメントシステム（PKMS）に変える生産性ツールを提供します。バックリンクビュー、タスク管理、ジャーナル、カンバンボード、Markdown 編集ツール、Mermaid、LaTeX 数式サポート、Jekyll / Hugo 風の静的サイト公開機能を含みます。
...
- **TODO ONLY** - 未完了のタスクのみ表示（デフォルトでオン）
- **WAITING ONLY** - `#W` タグ付きのタスクのみ表示
...
サイドバーの**カレンダー**パネルは、ジャーナルインジケーター付きの当月を表示します。任意の日をクリックしてジャーナルエントリを開きます。詳細は[カレンダー](#calendar)を参照。
...
これらのコマンドは、ファイルの YAML フロントマターのパブリッシュ関連フィールドを切り替えまたは循環させます。詳細は[静的サイトの公開](#as-notes-から-html-への公開html-変換)を参照。
...
#### フロントマタープリセット *（フロントマターがまだ存在しないファイル）*

これらのコマンドは、現在のファイルに YAML フロントマターブロックがない場合にのみ表示されます。編集しやすいタブストッププレースホルダー付きの完全なフロントマターテンプレートを挿入します。

| コマンド | アクション |
|---|---|
| **Front Matter: Blog Post** | ブログ投稿用のフロントマターを挿入：`title`、`description`、`date`（今日）、`author`、`public: true`、`layout: blog` |
| **Front Matter: Docs Page** | ドキュメントページ用のフロントマターを挿入：`title`、`description`、`public: true`、`layout: docs`、`order` |
| **Front Matter: Wiki Page** | Wiki ページ用のフロントマターを挿入：`title`、`description`、`public: true` |
...
インラインエディタ設定の完全なリストについては、[設定](#settings) を参照してください。
...
| カスタム日付形式 | `YYYY`、`YY`、`MM`、`DD`、`HH`、`mm`、`ss` トークンの任意の組み合わせ | `{{DD/MM/YYYY}}` → `18/03/2026`、`{{DD/MM/YY}}` → `18/03/26` |
...
スラッシュコマンドメニュー（`/`）のすべてのテーブル操作は Pro 機能です。無料ユーザーには **(Pro)** 付きでリスト表示されます - 表示はされますが、ライセンスがアクティブになるまでブロックされます。
...
### KeePass パスワードセーフ（Pro）

Pro ユーザーは、**KeePass Password Safe** サイドバーから標準的な **KeePass KDBX 4**（`.kdbx`）パスワードセーフを開いて編集・作成できます - 同じファイルは KeePassXC やその他の KeePass アプリでも開けます。マスターパスワード（オプションでキーファイルを追加）でロック解除し、グループとエントリを参照し、自動クリアされるクリップボードで認証情報とワンタイムコードをコピーし、エントリ（フィールド、カスタムフィールド、タグ、認証キー、添付ファイル、アイコン、有効期限、履歴）を保存オンデマンドのエディタで編集できます。新しいセーフはファイルごとにランダムなソルトを使用する Argon2id を使用します。詳細とセキュリティ上の考慮事項については [KeePass Password Safe](https://docs.asnotes.io/keepass-password-safe.html) を参照してください。
...
### 依存関係のインストールと全パッケージのビルド

ルートの `package.json` は、すべてのパッケージにまたがって作業するための便利なスクリプトを提供します。リポジトリルートから：
...
> **注意：** `vs-code-extension` ビルドは `../publish/src/convert.ts` を直接バンドルするため、拡張機能をビルドする前に `publish/`（およびその `common/` への依存）の依存関係をインストールしておく必要があります。リポジトリルートから `npm run install:all` を使用して、すべてを正しい順序でインストールしてください。
...
> **前提条件：** 最初に `common/` と `publish/` の依存関係をインストールしておく必要があります（`npm --prefix common install && npm --prefix publish install`）。または、リポジトリルートから `npm run install:all` を使用してください。
...
サイト全体のレイアウトが `blog` の場合（または設定で `blogIndex: true` が設定されている場合）、自動生成されるインデックスページはブログ対応レイアウトを使用します：

1. **最近の投稿** — 最も新しい投稿（`recentCount` で設定可能、デフォルト 3）を、タイトル、日付、著者、説明、オプションのヒーロー画像付きのカードとして表示
2. **今年** — 今年の残りの投稿をコンパクトな日付 + タイトルのリストとして表示
3. **年別アーカイブ** — 過去の年に対して自動生成された `archive-YYYY.html` ページへのリンク
4. **日付なし** — フロントマターに `date` がない投稿は最下部にリスト表示

#### 公開用フロントマターフィールド

| フィールド | 型 | 説明 |
|---|---|---|
| `public` | boolean | ページを公開出力に含める |
| `title` | string | ページタイトル（デフォルトはファイル名） |
| `description` | string | SEO 用のメタ説明 |
| `date` | string | 公開日（`YYYY-MM-DD`）— ブログのソート、RSS、サイトマップに使用 |
| `author` | string | 投稿の著者 — ブログレイアウトと RSS フィードに表示 |
| `image` | string | ヒーロー/サムネイル画像のパス — ブログレイアウトとブログインデックスカードに表示 |
| `layout` | string | ページごとのレイアウトの上書き（`docs`、`blog`、`minimal`） |
| `order` | number | ナビゲーションのソート順 |
| `draft` | boolean | `--include-drafts` が設定されていない限り出力から除外 |
| `assets` | boolean | 参照された画像を出力にコピー |
| `retina` | boolean | Retina 画像サイジングを有効化 |

#### ブログ設定オプション

| オプション | 型 | デフォルト | 説明 |
|---|---|---|---|
| `siteTitle` | string | `"AS Notes Site"` | RSS フィードとブログインデックスで使用されるサイトタイトル |
| `recentCount` | number | `3` | ブログインデックスにカードとして表示される最近の投稿の数 |
| `blogIndex` | boolean | `false` | ブログスタイルのインデックス生成を使用（`layout: "blog"` の場合は自動的に有効化） |
