# AS Notes (PKMS, 정적 사이트 게시 및 블로깅 VS Code 확장 프로그램)
...
**AS Notes는 마크다운과 `[[wikilink]]` 편집 기능을 [VS Code](https://code.visualstudio.com/) 및 호환 편집기(예: [Antigravity](https://antigravity.google/), [Cursor](https://cursor.com/), [Windsurf](https://windsurf.com/))에 직접 통합하여 노트, 문서, 블로그, 위키를 작성할 수 있게 합니다. AS Notes는 Obsidian이나 Logseq 같은 PKMS 앱의 편집기 내 대안입니다**
...
저널 파일은 전용 `journals/` 폴더(설정 가능)에 `YYYY-MM-DD.md`로 생성됩니다. 새 페이지는 템플릿 폴더(기본값: `templates/`)의 `Journal.md` 템플릿에서 생성됩니다. `Journal.md`를 편집하여 자신만의 섹션과 프롬프트를 추가하세요. 모든 템플릿 플레이스홀더가 지원됩니다 -- [Templates](#템플릿-pro)를 참조하세요.

사이드바의 **캘린더** 패널은 저널 표시기가 있는 현재 월을 표시합니다. 아무 날이나 클릭하여 저널 항목을 엽니다. 자세한 내용은 [Calendar](#calendar)를 참조하세요.

> **참고:** 일일 저널은 초기화된 워크스페이스(`.asnotes/` 디렉토리)가 필요합니다. [Getting started](#시작하기)를 참조하세요.
...
이 명령은 파일의 YAML 프론트매터에서 게시 관련 필드를 전환하거나 순환합니다. 자세한 내용은 [Publishing a Static Site](#as-notes에서-html로-게시-html-변환)를 참조하세요.
...
#### 프론트매터 프리셋 *(기존 프론트매터가 없는 파일)*

이 명령은 현재 파일에 YAML 프론트매터 블록이 없을 때만 나타납니다. 손쉬운 편집을 위한 탭스톱 플레이스홀더가 포함된 완전한 프론트매터 템플릿을 삽입합니다.

| 명령 | 동작 |
|---|---|
| **Front Matter: Blog Post** | 블로그 게시물용 프론트매터 삽입: `title`, `description`, `date` (오늘), `author`, `public: true`, `layout: blog` |
| **Front Matter: Docs Page** | 문서 페이지용 프론트매터 삽입: `title`, `description`, `public: true`, `layout: docs`, `order` |
| **Front Matter: Wiki Page** | 위키 페이지용 프론트매터 삽입: `title`, `description`, `public: true` |
...
| 명령 | 동작 |
|---|---|
| **Template** | 템플릿 폴더에서 템플릿 빠른 선택 목록을 열고 선택한 템플릿을 커서 위치에 삽입. 플레이스홀더 지원 ([Templates](#템플릿-pro) 참조) |
...
### 인라인 편집기 마크다운 스타일링, Mermaid 및 LaTeX 렌더링 (Pro)
...
자세한 내용은 [Inline Editor Markdown Styling, Mermaid and LaTeX Rendering](https://docs.asnotes.io/inline-markdown-editing-mermaid-and-latex-rendering.html)을 참조하세요.
...
인라인 편집기 설정의 전체 목록은 [Settings](#settings)를 참조하세요.
...
| 사용자 정의 날짜 형식 | `YYYY`, `YY`, `MM`, `DD`, `HH`, `mm`, `ss` 토큰의 모든 조합 | `{{DD/MM/YYYY}}` → `18/03/2026`, `{{DD/MM/YY}}` → `18/03/26` |
...
테이블 명령의 전체 목록은 [Slash Commands](#슬래시-명령)를 참조하세요.
...
Pro 사용자는 표준 **KeePass KDBX 4** (`.kdbx`) 비밀번호 금고를 **KeePass Password Safe** 사이드바에서 열고, 편집하고, 생성할 수 있습니다 - 동일한 파일이 KeePassXC 및 기타 KeePass 앱에서 열립니다. 마스터 비밀번호(선택적 키 파일 포함)로 잠금 해제하고, 그룹과 항목을 탐색하고, 자동 삭제 클립보드로 자격 증명과 일회용 코드를 복사하고, 저장 시점 지정 편집기에서 항목(필드, 사용자 정의 필드, 태그, 인증기 키, 첨부 파일, 아이콘, 만료 및 기록)을 편집합니다. 새 금고는 파일별 랜덤 솔트가 있는 Argon2id를 사용합니다. 자세한 내용과 보안 고려 사항은 [KeePass Password Safe](https://docs.asnotes.io/keepass-password-safe.html)를 참조하세요.
...
- **`.asnotesignore` 패턴** - AS Notes 루트의 `.asnotesignore` 패턴과 일치하는 파일은 인덱스에서 제외됩니다. 위의 [Excluding files from the index](#인덱스에서-파일-제외)를 참조하세요.
...
루트 `package.json`은 모든 패키지에서 작업하기 위한 편의 스크립트를 제공합니다. 저장소 루트에서:
...
> **참고:** `vs-code-extension` 빌드는 `../publish/src/convert.ts`를 직접 번들링하므로, 확장 프로그램을 빌드하기 전에 `publish/`(및 `common/`에 대한 의존성)의 의존성이 설치되어 있어야 합니다. 올바른 순서로 모든 것을 설치하려면 저장소 루트에서 `npm run install:all`을 사용하세요.
...
> **전제 조건:** `common/`과 `publish/`의 의존성이 먼저 설치되어 있어야 합니다(`npm --prefix common install && npm --prefix publish install`). 또는 저장소 루트에서 `npm run install:all`을 사용하세요.
...
사이트 전체 레이아웃이 `blog`인 경우(또는 설정에서 `blogIndex: true`가 설정된 경우), 자동 생성된 인덱스 페이지는 블로그 인식 레이아웃을 사용합니다:

1. **최근 게시물** — 가장 최근 게시물(`recentCount`로 설정 가능, 기본값 3)이 제목, 날짜, 작성자, 설명 및 선택적 히어로 이미지가 있는 카드로 표시
2. **현재 연도** — 현재 연도의 나머지 게시물이 간결한 날짜 + 제목 목록으로
3. **연도별 아카이브** — 이전 연도의 자동 생성된 `archive-YYYY.html` 페이지로 연결되는 링크
4. **날짜 없음** — 프론트매터에 `date`가 없는 게시물은 하단에 나열

#### 게시용 프론트매터 필드

| 필드 | 유형 | 설명 |
|---|---|---|
| `public` | boolean | 게시된 출력에 페이지 포함 |
| `title` | string | 페이지 제목 (기본값은 파일 이름) |
| `description` | string | SEO용 메타 설명 |
| `date` | string | 게시 날짜 (`YYYY-MM-DD`) — 블로그 정렬, RSS, 사이트맵에 사용 |
| `author` | string | 게시물 작성자 — 블로그 레이아웃과 RSS 피드에 표시 |
| `image` | string | 히어로/썸네일 이미지 경로 — 블로그 레이아웃과 블로그 인덱스 카드에 표시 |
| `layout` | string | 페이지별 레이아웃 재정의 (`docs`, `blog`, `minimal`) |
| `order` | number | 탐색 정렬 순서 |
| `draft` | boolean | `--include-drafts`가 설정되지 않는 한 출력에서 제외 |
| `assets` | boolean | 참조된 이미지를 출력으로 복사 |
| `retina` | boolean | 레티나 이미지 크기 조정 활성화 |

#### 블로그 설정 옵션

| 옵션 | 유형 | 기본값 | 설명 |
|---|---|---|---|
| `siteTitle` | string | `"AS Notes Site"` | RSS 피드와 블로그 인덱스에 사용되는 사이트 제목 |
| `recentCount` | number | `3` | 블로그 인덱스에 카드로 표시되는 최근 게시물 수 |
| `blogIndex` | boolean | `false` | 블로그 스타일 인덱스 생성 사용 (`layout: "blog"`일 때 자동 활성화) |

전체 문서는 [Publishing a Static Site](https://docs.asnotes.io/publishing-a-static-site.html)를 참조하세요
...
AS Notes용 [agent skill](https://skills.sh/)을 사용할 수 있습니다. 설치하면 AI 비서(GitHub Copilot, Claude 등)에 확장 프로그램의 전체 지식 - 위키 링크 구문, 명령, 설정, 키보드 단축키 등을 제공할 수 있습니다.
