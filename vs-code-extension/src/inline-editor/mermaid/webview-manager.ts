...
export class MermaidWebviewManager {
  ...
  private ensureWebviewThenSwitchBack(): void {
    ...
    vscode.commands
      .executeCommand('as-notes.mermaidRenderer.focus')
      .then(
        () => {
          ...
          Promise.race([
            ...
            .catch((err: unknown) => {
              if (err instanceof Error && err.message === 'timeout') {
                getActiveLogger().warn('MermaidWebview', 'Webview not ready after opening view');
              }
              this.initTimeoutId = setTimeout(() => {
                vscode.commands.executeCommand('workbench.view.explorer');
                this.initTimeoutId = undefined;
        ...
        (err: unknown) => {
          if (err !== undefined) {
            getActiveLogger().warn('MermaidWebview', `Failed to focus view: ${formatLogError(err)}`);
          }
        }
      );
  }
...
class MermaidWebviewViewProvider implements vscode.WebviewViewProvider {
  public static readonly viewType = 'as-notes.mermaidRenderer';
